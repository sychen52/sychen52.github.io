...
project is slowing down and even
[[https://twimlai.com/whats-next-for-fast-ai-w-jeremy-howard/][Jeremy
...
* Ahead of Time (AOT) Compilation vs. Just In Time (JIT) Compilation vs. Interpreter
  ...
  :END:
As a rule of thumb, static typing language can be AOT compiled while
dynamic language usually runs on an interpreter, and sometimes with a
...
dynamic typing languages cannot only to optimized at runtime when the
type is known with a JIT. Julia is none of this. It is
[[https://www.youtube.com/watch?v=XWIZ_dCO6X8][just ahead of time
(JAOT)]] compiled. The pro is that you have a deeply optimized dynamic
