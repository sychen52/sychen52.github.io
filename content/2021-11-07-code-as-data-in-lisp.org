...
* Curse of LISP
Different people may have different understandings of this. My understanding is more like a "Curse of Macro", so it applies to other languages that has similar macro as LISP, such as Julia. When you compose, all you need to care about is the input and output of the functions. Inside each function, it can go as complicated as it wants, while from the outside, you, as a composer, don't need to peak into the mess inside the function. If you work with a strongly typed language, the language even gives you the input and output type of a function. It is like a quick start manual, which gives you a jump start at composing.

On the other hand, if you want to compose macros, you mostly want to peak into the function/code. Otherwise, why not just do function composition. In this case, since it is a direct peak into the mess, and there is not quick start manual for you, it makes compose macros hard. In the end, each package/module just build their only macros which do not work well with others. The difficulty of compose is what I think is the curse of LISP.

In other words, function is a good way to abstract (with the help of input and output type), while macro is not good at abstract. If components are not abstracted well, it will become hard to compose eventually.
