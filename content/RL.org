...
- why do we want the policy before and after updating to be close.
  ...
  - cs285 lecture: TRPO and PPO have a new "loss function". For this loss function to be the same as the one used mentioned before. There is one constraint: π_θ' is close to π_θ => p_θ'(sₜ) is close to p_θ(sₜ)
