...
- A small grad update of policy NN does NOT necessarily mean the resulting difference in policy is small. This means a bad step in policy gradient can collapse the policy
- Maximum a new surrogate advantage, with a constraint that KL divergence of the policy before and after update is less than a threshold.
...
** PPO-Clip
- same surrogate advantage function as TRPO, which contain a ratio between new and old policy.
...
- DDPG is the Q-learning for continuous space.
  ...
  - To learn this optimal policy, to loss function is just to max Q* (while the parameters in the Q* NN being fixed.).
** Tricks:
*** Experience Replay
- For previous on-policy algorithms, such as VPG, TRPO, and PPO. The loss function and the gradient term has a expection term sample over current policy. This makes the old experience unusable.
- For Q-learning algorithm, the loss function is mean-squared Bellman error (MSBE), and this equation should be satisfied with any action you sampled.
*** Target Networks
- There is a "max Q*" term inside Bellman equation.
  - As mentioned about, to estimate the max part in a continuous action space, you need to use the estimated optimal policy.
  - if both Q* in the Bellman equation are the same Q* estimated by NN, it will be unstable. Therefore, they use a target network for Q* and a target network for optimal policy. These two are used for estimating "max Q*". The target networks is just a running average of the normal NN.

* [[https://spinningup.openai.com/en/latest/algorithms/td3.html][Twin Delayed DDPG]]
** Three tricks
- learn 2 Q-function and use the smaller of the two Q-value
- update policy less frequently than Q-function
- add noise to target action to smooth the policy
