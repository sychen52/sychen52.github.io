...
- For the following example, in =S{Int64}=, and =s.a= could be =Vector{Int64}= or =SparseVector{Int64}=. If it is a mutible struct, you can even change =s.a='s type at runtime after creation. Therefore, the compiler has no way to optimize it.
  #+begin_src julia
    struct S{T}
        a::AbstractVector{T}
    end
...
* where
- =Vector{T} where T <: Real= means Union{Vector{T} for all T belong to Real}.
- Unionall: =Vector= means =Vector{T} where T <: Any=
- In function signature, =function f(x::T) where {T}= just tells the parser, T is template/parametric. =function f(x::T) where {T<:Real}= can be used to put some constraint on the type.
- On the other hand, there is no where for struct to put constraint. You directly put constraint in the ={}=. For example, =struct S{T<:Real}=
